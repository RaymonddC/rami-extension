          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-600 dark:text-neutral-400 -mt-1">Untangle Your Mind</p>
              </div>
            </div>
            <PersonaSelector selectedPersona={preferences?.persona || 'strategist'} onSelect={(persona) => setPreferences({ ...preferences, persona })} compact />
          </div>
                ...
                <div className="flex items-center justify-between bg-white dark:bg-neutral-850 p-4 rounded-lg border border-neutral-200 dark:border-neutral-700">
                  <div className="flex items-center gap-3">
                    ...
                    {selectedReading && (
                      ...
                      >
                        {selectedReading.usedAI === false || selectedReading.generationMethod === 'fallback' ? '⚡ Fallback Mode' : '🤖 AI Generated'}
                      </span>
                    )}
                  </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
        ...
        >
          ...
          <div className="px-6 py-4 bg-neutral-50 dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
            <div className="text-xs text-neutral-600 dark:text-neutral-400">AI-generated comprehensive summary • {reading.generationMethod === 'fallback' ? '📝 Fallback mode' : '🤖 AI-powered'}</div>
            <button
              onClick={() => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {readings.map((reading) => (
        <motion.div key={reading.id} whileHover={{ y: -4 }} className={`card text-left relative group cursor-pointer ${selected?.id === reading.id ? 'ring-2 ring-primary-500' : ''}`} onClick={() => onSelect(reading)}>
          {/* Delete Button */}
